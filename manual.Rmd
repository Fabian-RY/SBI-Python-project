...
The input is a folder with 2 or more interactions betweem 2 proteins, a protein and DNA/RNA strand or 2 single DNA strands (which may or may not form a double strand). Two structures are considered to be interacting where the minimum distance between them is less than 10 Armstrong. However, in lesser distances, forces between atoms are strong enough to produce changes between the chains and force them to adopt a different conformation, an thus, a realistic model must keep the residues at a real distance to consider it correct.
...
It also has a graphical interface wich accepts the same paramenters but grphically with message boxes and graphical stuff. It's an independent executable, so the CLI one can be used for automation without confusion
...
You can indicate the minimum distance to consider that two proteins do not clash. The molel will discard interactions with too much atoms at lesser distance than the value indicated in Argmstrongs. Being too strictive can discard some interactions, but being so flexible can force chains to overlap and not being energetically favorable.
...
An additional file with the stoichiometry can be given to the program to make sure that the final protein will contain no more than the indicated chains
